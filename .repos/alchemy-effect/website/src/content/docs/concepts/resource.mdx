...
Output attributes are available as [`Output`](/concepts/outputs)
...
See [Inputs and Outputs](/concepts/outputs) for the full set of
...
inside a [Stack](/concepts/stack) doesn't either; it just **registers
the resource on the stack** and hands you back a typed
[`Output`](/concepts/outputs) reference for its attributes:
...
[Resource Lifecycle](/concepts/resource-lifecycle) for what happens
after registration.
...
stages](/guides/shared-database) guide for the canonical
walkthrough.
...
The resource's [provider](/concepts/provider) decides which property
changes trigger replacement vs in-place update (via
[`diff`](/concepts/provider#diff)). For the full lifecycle
...
See [Writing a Custom Resource Provider](/guides/custom-provider)
...
[Provider](/concepts/provider#lifecycle-operations).

## The resource graph

Passing an `Output` from one resource as input to another draws an
...
Cycles (Worker A binds Worker B, Worker B binds Worker A) are
handled with a two-phase plan — see
...
IaC engines reject these — alchemy resolves them by splitting each
[Platform](/concepts/platform) resource into two pieces:

- A **class** that acts as the Tag (the identity / declaration)
- A **`.make(...)`** Layer that supplies the runtime implementation

The class can be referenced before its implementation exists, so two
Workers can name each other in their handlers without a hard
ordering constraint.

For a circular Worker pair, the resource graph contains edges in
both directions:
...
To compose Workers that reference each other, provide both Layers
to your Stack with `Effect.provide`. See the
[Circular Bindings guide](/guides/circular-bindings) for a complete
