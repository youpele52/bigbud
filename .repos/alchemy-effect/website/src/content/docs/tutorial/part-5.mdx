...
- **Part 5** — Mint scoped CI credentials with
  `Cloudflare.AccountApiToken`, push them to GitHub with
  `GitHub.Secret`, and deploy from Actions

## What's next

- Go deeper on Cloudflare in the
  [Cloudflare track](/tutorial/cloudflare/durable-objects) — layer
  Durable Objects, hibernatable WebSockets, Containers, Workflows,
  and Queues onto the Worker you built
- Read the [Resource Lifecycle](/concepts/resource-lifecycle)
  guide for all CLI flags and options
- Explore the [CI guide](/guides/ci) for the AWS equivalents
  (OIDC and access keys) and more workflow patterns
- Check out the [Testing reference](/concepts/testing) for more
