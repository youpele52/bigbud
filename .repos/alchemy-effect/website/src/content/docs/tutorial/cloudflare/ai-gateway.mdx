...
`Cloudflare.AiGateway.bind(Gateway)` returns a typed client whose
...
Every prop on `Cloudflare.AiGateway` maps to an update API call — no
...
- A `Cloudflare.AiGateway` resource with caching, logging, and (when
...
API or RPC handler — see the [Effect AI guide](/guides/effect-ai).
