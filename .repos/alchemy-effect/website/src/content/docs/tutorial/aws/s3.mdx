...
The Lambda you built in [Deploy a Lambda Function](/tutorial/aws/lambda)
serves a static string. Real services need somewhere to put data.
In this part you'll add an **S3 Bucket** for blob storage and bind
**PutObject** + **GetObject** to the function — Alchemy will mint
a least-privilege IAM policy from the bindings you actually use.
...
Both `bind(bucket)` calls return a callable Effect: `putObject`
...
Each `.bind(...)` call is **1:1 with an IAM policy statement**
...
sight. Next we'll [react to S3 events](/tutorial/aws/s3-events)
so the same function can be notified whenever a new object lands
in the bucket — wired through the same binding pipeline.
