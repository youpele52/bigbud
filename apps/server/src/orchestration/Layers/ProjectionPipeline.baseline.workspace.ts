...
import { toPersistenceDecodeCauseError } from "../../persistence/Errors.ts";
...
export const verifyCandidateInWorkspace = (input: {
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const context = yield* Layer.build(workspaceLayer.pipe(Layer.provideMerge(persistence)));
      const workspaceSql = yield* Effect.service(SqlClient.SqlClient).pipe(Effect.provide(context));
      const baselines = yield* Effect.service(ProjectionBaselineRepository).pipe(
        Effect.provide(context),
      );
      const state = yield* Effect.service(ProjectionStateRepository).pipe(Effect.provide(context));
  ...
  `;
      ...
      const replayPage = (events: ReadonlyArray<import("@bigbud/contracts").OrchestrationEvent>) =>
      ...
      while (cursor < input.candidate.sequence) {
        ...
        const events = replay.events.filter((event) => event.sequence <= input.candidate.sequence);
        if (events.length === 0) {
          return yield* toPersistenceDecodeCauseError("ProjectionBaseline.verify:missingReplay")(
      ...
      yield* fs.remove(workspacePath, { force: true });
      return matches;
    }),
  );
