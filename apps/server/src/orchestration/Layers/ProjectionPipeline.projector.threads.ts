...
import { projectThreadQueueControlEvent } from "./ProjectionPipeline.projector.threadQueueControl.ts";
...
export function makeThreadsProjector(
  ...
  const apply = Effect.fn("applyThreadsProjection")(function* (
    ...
    if (yield* projectThreadQueueControlEvent({ event, repository: projectionThreadRepository }))
      return;
    switch (event.type) {
      case "thread.created":
      ...
      case "thread.deleted": {
        const threadIds = event.payload.threadIds ?? [event.payload.threadId];
