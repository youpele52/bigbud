...
describe("ProviderCommandReactor", () => {
  ...
  it("applies thread runtime updates and honors the next turn captured runtime", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    // The next turn explicitly requested full access; it must not inherit the new default.
