...
export function useMascotAnimation(isHovered: boolean): {
  ...
  useEffect(() => {
    const holdAttention = () => {
      ...
      }, ATTENTION_HOLD_DURATION_MS);
    };
    return subscribeMascotAttention(holdAttention);
  }, []);
  ...
  const animation = selectMascotAnimation({
    agentUncertain: agentIsUncertain,
    isCelebrating,
    hasAttention,
    isHovered,
    isGreeting,
    assistantIsActivelyTyping,
    workAnimation: workState.animation,
  });
