...
export function SidebarFavoritesSection({
  ...
  return (
    <SidebarGroup className="px-2 py-0">
      <SidebarMenu>
        <div className="group/project-header relative" onContextMenu={headerProps?.onContextMenu}>
          <SidebarMenuButton
            render={<div />}
            size="sm"
            className="h-7 gap-2 px-2 py-0 text-left hover:bg-accent group-hover/project-header:bg-accent group-hover/project-header:text-sidebar-accent-foreground"
            onClick={() => onExpandedChange(!isExpanded)}
          >
            <button
              type="button"
              className="flex min-w-0 flex-1 touch-pan-y items-center gap-2 text-left"
              {...headerProps?.dragHandleProps?.attributes}
              {...headerProps?.dragHandleProps?.listeners}
            >
              <PinIcon
        ...
        {isExpanded && renderedFavorites.length > 0 ? (
          <SidebarMenuSub className="my-0 ml-2 mr-1 gap-0.5 overflow-hidden pl-3 pr-1 py-0">
            {visibleFavorites.map((entry) => (
              <SidebarThreadRow
                ...
                handleMultiSelectContextMenu={sharedProjectItemProps.handleMultiSelectContextMenu}
